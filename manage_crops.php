...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_crop']) || isset($_POST['update_crop'])) {
        ...
        // image handling - always convert to base64 and store in database to guarantee
        // persistence across restarts.  Filesystem storage proved unreliable in some
        // deployment environments, so we no longer depend on it.
        $image_url = '';
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK && $_FILES['image']['size'] > 0) {
            ...
            if (empty($error)) {
                // always encode the uploaded image as data URI
        ...
        // Delete crop (image is stored in database as base64 so no file cleanup needed)
